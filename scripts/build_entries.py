...
CSS_LINUX_SOURCE = ROOT / "css" / "linux.css"

LINUX_SOURCE_DIR = ROOT / "linux"
...
def copy_css() -> None:
    """
    Copy all website stylesheets into build/css/.
    """
    ...
    for source in css_files:
        ...
        if not source.exists():
            raise SystemExit(
                f"ERROR: {source.relative_to(ROOT)} not found."
            )

        destination = BUILD_CSS_DIR / source.name
...
def copy_linux() -> None:
    """
    Copy the complete linux/ knowledge section into build/linux/.

    Everything inside linux/ is treated as static website content.
    """

    if not LINUX_SOURCE_DIR.exists():
        raise SystemExit(
            f"ERROR: {LINUX_SOURCE_DIR.relative_to(ROOT)} not found."
        )
    ...
    print(
        f"Copied: "
        f"{LINUX_SOURCE_DIR.relative_to(ROOT)}"
        f" -> "
        f"{BUILD_LINUX_DIR.relative_to(ROOT)}"
    )
...
def main() -> int:
    ...
    if not LINUX_SOURCE_DIR.exists():
        raise SystemExit(
            "ERROR: linux/ directory not found."
        )

    # ------------------------------------------------------------
    # Start with a clean build directory.
    #
    # This ensures deleted entries/pages disappear from deployment.
    # ------------------------------------------------------------
    ...
    # ------------------------------------------------------------
    # Copy the Linux knowledge section.
    # ------------------------------------------------------------
